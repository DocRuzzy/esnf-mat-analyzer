...
class MultiScaleUniformityAnalyzer:
    ...
    def __init__(
        self, wavelet: str = "db4", levels: int = 4, max_coefficients: int = 100000
    ):
    ...
    def analyze_multiscale_uniformity(
        self,
        ...
        if not np.any(roi_mask):
            # No valid ROI region
            self.logger.warning(
                "ROI mask has no True values - no valid analysis region"
            )
            return {"scale_0_uniformity": 0.0}
        ...
        if len(valid_thickness_in_roi) > 0:
            roi_thickness_range = (
                np.min(valid_thickness_in_roi),
                np.max(valid_thickness_in_roi),
            )
        else:
            roi_thickness_range = (0, 0)

        self.logger.debug(
            f"Multiscale analysis: ROI covers {roi_percentage:.1f}% of image ({roi_true_pixels} pixels)"
        )
        self.logger.debug(
            f"Thickness map range: {thickness_range}, ROI thickness range: {roi_thickness_range}"
        )
        ...
        for level in range(self.levels + 1):  # Include level 0 through self.levels
                    ...
                    if downsample_factor_y > 0 and downsample_factor_x > 0:
                        ...
                        if downsampled_mask.shape != scale_data.shape:
                            downsampled_mask = downsampled_mask[
                                : scale_data.shape[0], : scale_data.shape[1]
                            ]
                    else:
                        downsampled_mask = roi_mask
                else:
                ...
                if isinstance(detail_coeffs, tuple) and len(detail_coeffs) == 3:
                    ...
                    for coeff_array in [cH, cV, cD]:
                        if coeff_array.shape != roi_mask.shape:
                            # Downsample ROI mask to match coefficient dimensions
                            downsample_factor_y = (
                                roi_mask.shape[0] // coeff_array.shape[0]
                            )
                            downsample_factor_x = (
                                roi_mask.shape[1] // coeff_array.shape[1]
                            )
                            if downsample_factor_y > 0 and downsample_factor_x > 0:
                                downsampled_mask = roi_mask[
                                    ::downsample_factor_y, ::downsample_factor_x
                                ...
                                if downsampled_mask.shape != coeff_array.shape:
                                    downsampled_mask = downsampled_mask[
                                        : coeff_array.shape[0], : coeff_array.shape[1]
                                    ]
                            else:
                                downsampled_mask = roi_mask
                        else:
            ...
            if len(valid_data) > max_coefficients:
                self.logger.warning(
                    f"Subsampling {len(valid_data)} coefficients to {max_coefficients} for scale {level} to avoid memory issues."
                )
                indices = np.random.choice(
                    len(valid_data), max_coefficients, replace=False
                )
            ...
            self.logger.debug(
                f"Scale {level}: {len(valid_data)} valid data points, mean={mean_val:.6f}, std={std_val:.6f}"
            )
            ...
            if level == 0:
                ...
                if abs(mean_val) > 1e-12:
                    ...
                    self.logger.debug(
                        f"Scale {level}: CV={cv:.6f}, uniformity={uniformity_value:.6f}"
                    )
                else:
                    ...
                    if std_val < 1e-12:
                        ...
                        self.logger.debug(
                            f"Scale {level}: Zero variance case, uniformity=1.0"
                        )
                    else:
            ...
            scale_uniformity[f"scale_{level}_uniformity"] = np.clip(
                uniformity_value, 0.0, 1.0
            )
        ...
        if not scale_uniformity:
            ...
            self.logger.debug(
                f"Multiscale analysis completed with {len(scale_uniformity)} valid scales: {list(scale_uniformity.keys())}"
            )

        return scale_uniformity

    def create_multiscale_heatmap(
        self, thickness_map: np.ndarray, roi_mask: np.ndarray, window_size: int = 64
    ) -> Dict[str, np.ndarray]:
        ...
        try:
            ...
            self.logger.warning(
                "PyWavelets not available, cannot create multiscale heatmap"
            )
            return {}
        ...
        for y in range(half_window, height - half_window, half_window):
            for x in range(half_window, width - half_window, half_window):
                ...
                local_results = self.analyze_multiscale_uniformity(
    ...
    def _fallback_multiscale_analysis(
        ...
        if not np.any(roi_mask):
            # No valid ROI region
            self.logger.warning(
                "ROI mask has no True values - no valid analysis region (fallback)"
            )
            return {"scale_0_uniformity": 0.0}
        ...
        roi_percentage = (roi_true_pixels / roi_mask.size) * 100
        self.logger.debug(
            f"Fallback multiscale analysis: ROI covers {roi_percentage:.1f}% of image ({roi_true_pixels} pixels)"
        )
        ...
        for level in range(self.levels + 1):
            ...
            # Downsample the image and mask (views, not copies)
            if scale_factor == 1:
            ...
            # Streaming/statistics helper to avoid large temporaries
            def compute_streaming_stats(map_arr, mask_arr=None, max_samples=1000000):
                """Compute mean and std in a streaming fashion over masked data.

                If mask_arr provided and matches shape, iterate row-wise over masked values.
                If total valid samples exceed max_samples, reservoir-sample up to max_samples.
                ...
                # Generator over valid values
                def value_iter():
                    if mask_arr is not None and mask_arr.shape == map_arr.shape:
                        # iterate row-wise to avoid large single allocations
                        for row_map, row_mask in zip(map_arr, mask_arr):
                            vals = row_map[row_mask]
                            for v in vals:
                                if np.isfinite(v):
                                    yield float(v)
                    else:
                        # fallback: iterate flattened array in chunks
                        flat = map_arr.ravel()
                        for v in flat:
                            if np.isfinite(v):
                                yield float(v)

                # Simple reservoir sampling if data too large
                sample = []
                for i, val in enumerate(value_iter()):
                    if i < max_samples:
                        sample.append(val)
                    else:
                ...
                # Compute mean/std on the sample (safe size)
                arr = np.frombuffer(
                    np.array(sample, dtype=np.float64), dtype=np.float64
                )
                cnt = arr.size
                mean = arr.mean()
                std = arr.std()
            ...
            mean_val, std_val, cnt = compute_streaming_stats(
                downsampled_map, downsampled_mask, max_samples=self.max_coefficients
            )
            ...
            if level == 0:
                ...
                # Use robust estimates from streaming sample to compute range
                # For range we compute on the sample only to avoid full-array allocations
                # (This is conservative but prevents memory issues for large inputs)
                data_range = max(std_val * 2.0, 1e-12)
                normalized_std = std_val / data_range
                uniformity_value = 1.0 / (1.0 + normalized_std)
        ...
        if not scale_uniformity:
            ...
            self.logger.debug(
                f"Fallback multiscale analysis completed with {len(scale_uniformity)} valid scales: {list(scale_uniformity.keys())}"
            )
